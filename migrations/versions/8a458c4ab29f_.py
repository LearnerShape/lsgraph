...
def upgrade():
    ...
    op.add_column(
        "profile",
        sa.Column("previous_version", postgresql.UUID(as_uuid=True), nullable=True),
    )
    op.add_column("profile", sa.Column("type", sa.String(length=256), nullable=True))
    op.add_column(
        "profile", sa.Column("user_id", postgresql.UUID(as_uuid=True), nullable=True)
    )
    op.create_index(
        op.f("ix_profile_organization_id"), "profile", ["organization_id"], unique=False
    )
    ...
    op.create_index(
        op.f("ix_profile_skill_profile_id"),
        "profile_skill",
        ["profile_id"],
        unique=False,
    )
